...
val SkeletonBaseStyle = CssStyle.base {
    val base = if (colorMode == ColorMode.DARK) com.varabyte.kobweb.showcase.site.ui.theme.SiteColors.SkeletonBase else rgb(0xE0E4EC)
    Modifier
...
fun SkeletonCard() {
    ...
    Column(
        ...
        // Image placeholder
        SkeletonBox(
            Modifier.fillMaxWidth().height(180.px).borderRadius(0.px)
        )
        Column(Modifier.padding(1.25.cssRem).gap(0.75.cssRem)) {
            ...
            SkeletonBox(
                Modifier.fillMaxWidth().height(0.85.cssRem)
            )     // desc line 1
            SkeletonBox(
                Modifier.width(
                    80.percent
            ...
            Row(Modifier.margin(top = 0.5.cssRem).gap(0.4.cssRem)) {
                repeat(3) {
                    SkeletonBox(
                        Modifier.width(3.5.cssRem).height(1.3.cssRem).borderRadius(2.cssRem)
                    )
                }
            }
        }
